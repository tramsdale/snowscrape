...
# Import Lambda configuration if available
try:
    from lambda_config import DATA_DIR, IS_LAMBDA
    OUT_DIR = DATA_DIR
except ImportError:
...
STORAGE    = str(OUT_DIR / "snow_state.json")
...
def main():
    with sync_playwright() as p:
        ...
        # Generate OpenAI forecast with fresh data
        print("=== GENERATING AI FORECAST ===")
        try:
            generate_ai_forecast()
        except Exception as e:
            print(f"⚠️ AI forecast generation failed: {e}")
...
def generate_ai_forecast():
    """Generate OpenAI forecast using the freshly scraped data"""
    # Check if OpenAI API key is available
    api_key = os.getenv('OPENAI_API_KEY')
    if not api_key:
        print("⚠️ No OpenAI API key found, skipping AI forecast generation")
        return
    
    try:
        # Import the forecast generator
        from .forecast_generator import SkiForecastGenerator
        
        print("🤖 Initializing OpenAI forecast generator...")
        ...
        # Load the freshly scraped multi-elevation data
        elevation_data = generator.load_hourly_forecast(str(OUT_DIR))
        
        print(f"📊 Loaded data for {len(elevation_data)} elevation(s)")
        
        # Generate forecasts with ChatGPT
        print("🎯 Generating AI forecast analysis...")
        forecasts = generator.generate_forecast_with_chatgpt(elevation_data)
        
        if forecasts:
            # Save the forecasts in the output directory
            saved_files = generator.save_forecasts(forecasts, str(OUT_DIR / "generated_forecasts"))
            print(f"💾 AI forecast saved: {saved_files.get('html_file', 'Unknown')}")
            print("✅ AI forecast generation completed successfully")
        else:
            print("❌ No forecasts generated by ChatGPT")
            
    except ImportError as e:
        print(f"⚠️ Could not import forecast generator: {e}")
    except Exception as e:
        print(f"❌ AI forecast generation failed: {e}")
