...
try:
    from lambda_config import DATA_DIR, GENERATED_DIR, IS_LAMBDA
...
class SkiForecastGenerator:
    ...
    def load_hourly_forecast(self, data_dir: Optional[str] = None) -> Dict[str, List[Dict]]:
        """
        ...
        """
        # Use Lambda-compatible data directory if available
        if data_dir is None:
...
[Your complete Markdown forecast here]"""
    ...
    def save_forecasts(
        ...
        Args:
            ...
            output_dir: Directory to save files (uses Lambda-compatible default if None)
            
        Returns:
            Dictionary with file paths
        """
        # Use Lambda-compatible directory if not specified
