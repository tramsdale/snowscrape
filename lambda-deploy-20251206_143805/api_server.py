...
from lambda_config import DATA_DIR, LOG_DIR, STATIC_DIR, IS_LAMBDA
...
# Mount static files if directory exists (not needed in Lambda)
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "/scrape": "POST - Manually trigger scraping + AI forecast (auto runs every 2 minutes)",
            "/scrape/status": "GET - Check last scrape time and data file status"
        },
...
async def health_check():
    ...
    try:
        # Check for any of the actual forecast files created by scraper
        test_files = [
            DATA_DIR / "dynamic_forecast_mid.json",
            DATA_DIR / "hourly_forecast_mid.json", 
            DATA_DIR / "dynamic_forecast_top.json"
        ]
...
def get_cached_or_generate_forecast(hourly_data: List[Dict]) -> str:
    """Get pre-generated forecast from scraper (no longer generates on-demand)"""
    # Check for recent cached forecast generated by the scraper
    cached_forecast = get_recent_cached_forecast()
    if cached_forecast:
        print("✅ Using pre-generated AI forecast from scraper")
        return cached_forecast
    
    print("⚠️ No recent AI forecast found - will be generated during next scheduled scrape")
    return None
def get_recent_cached_forecast() -> str:
    """Check for recent cached forecast files (within last hour)"""
    try:
        # Look in the data directory where scraper saves forecasts
        forecasts_dir = DATA_DIR / "generated_forecasts"
...
def create_ski_themed_forecast_html(hourly_data: List[Dict],
    ...
    for event in snow_events:
        ...
        date_str = f"{event.get('day_name', '')} {event.get('day_num', '')} {current_month}"
...
async def trigger_scrape():
    """Trigger a fresh scrape of snow forecast data (synchronous for manual testing)"""
    try:
        ...
        if not snow_user or not snow_pass:
            raise HTTPException(
                status_code=500,
                detail="Scraping credentials not configured. Set SNOW_USER and SNOW_PASS environment variables."
            )
        
        # Import the scraper and run it in a thread to avoid asyncio conflicts
        import asyncio
        import concurrent.futures
        from snowscrape.scraper import main as scrape_main
        
        print("🔄 Starting snow forecast scrape...")
        loop = asyncio.get_event_loop()
        with concurrent.futures.ThreadPoolExecutor() as executor:
            await loop.run_in_executor(executor, scrape_main)
        print("✅ Snow forecast scraping completed successfully")
        
        return {
            "status": "completed",
            "message": "Snow forecast data scraped successfully",
            "timestamp": datetime.now().isoformat(),
            "note": "Automatic scraping and AI forecast generation runs every 2 minutes via CloudWatch schedule"
        }
...
async def get_scrape_status():
    """Get the current status of the scraping system based on file timestamps"""
    try:
        status = {
            "schedule": "rate(2 minutes) - DEBUG MODE",
            "files": {},
            "last_scrape": None,
            "data_freshness": "unknown"
        }
        
        # Check key data files and their timestamps
        key_files = [
            "dynamic_forecast_mid.json",
            "dynamic_forecast_top.json", 
            "dynamic_forecast_bot.json",
            "hourly_forecast_mid.json",
            "authentication_state.json"
        ]
        ...
        for filename in key_files:
            ...
            if file_path.exists():
                ...
                status["files"][filename] = {
                    ...
                    "age_minutes": int((datetime.now() - modified_time).total_seconds() / 60)
                }
                
                if latest_timestamp is None or modified_time > latest_timestamp:
                    latest_timestamp = modified_time
            else:
                status["files"][filename] = {"exists": False}
        
        if latest_timestamp:
            status["last_scrape"] = latest_timestamp.isoformat()
            age_minutes = int((datetime.now() - latest_timestamp).total_seconds() / 60)
            ...
            if age_minutes < 5:
                ...
                status["data_freshness"] = "moderate"
            else:
        ...
        raise HTTPException(status_code=500, detail=f"Error checking scrape status: {str(e)}")
